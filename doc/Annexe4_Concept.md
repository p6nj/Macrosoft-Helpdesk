...
|**Log_ticket_valides**|La table Log_ticket_valides est l'abstraction des journaux de création de tickets. Lorsqu'un utilisateur valide la création d'un ticket, une entrée y est faite. La table contient un identifiant d'entrée, l'identifiant de ticket auquel l'entrée de journal corresponds (clé étrangère avec la table Ticket), la date de validation du ticket, l'adresse IP qui a ouvert le ticket, et le niveau d'urgence renseigné. Il est à noter que ce niveau d'urgence n'est pas nécessairement le même que celui stocké dans le ticket qui a été créé, car le niveau d'urgence d'un ticket peut être modifié par l'administrateur web, contrairement à celui dans le journal de tickets validés. Il est possible de récupérer le login de l'utilisateur ayant créé le ticket en faisant une jointure avec la table Ticket.|
|**Log_connection_echec**|La table Log_connection_echec est l'abstraction des journaux de connections échouées. Lorsqu'une connection à un compte utilisateur est tentée sur le site et qu'elle est infructueuse, une entrée est faite dans cette table. Elle contient un identifiant d'entrée, la date de la tentative de connection, le login et le mot de passe tentés, ainsi que l'adresse IP ayant tenté de se connecter à un compte. Il est à noter que le login et le mot de passe ne correspondent pas nécessairement à un login ou un mot de passe existants dans la base de données.|

### Explications supplémentaires :

Nous aimerions expliquer quelques points supplémentaires sur le modèle de base de données utilisé. En particulier, la raison pour laquelle nous stockons les journaux d'activités dans cette base plutôt que sur un fichier système. Tout d'abord, centraliser le stockage des données sur une même base évite d'éparpiller les données et permets d'avoir un accès plus facile à ces dernières. Mais surtout, cela permets des comparaisons faciles entre les données des journaux et les données du modèle. C'est notamment pour cela que, bien que la table Log_connection_echec n'ait aucune relation avec d'autres tables, elle n'en reste pas moins légitime.

Par exemple, il est possible en mettant ces journaux sous la forme de tables de regarder si le login tenté dans le journal d'activité de connections échouées corresponds bien à un login existant dans la table Utilisateur. Si c'est le cas, il pourrait s'agir d'un simple oubli de mot de passe. Ou encore, si on voit beaucoup de lignes correspondant à cette requête, on peut suspecter une tentative d'attaque. 

Il nous semble également important d'expliquer la relation entre Log_tickets_valides et Ticket. Bien qu'il s'agisse d'une relation où pour chaque élément d'une table il en existe un seul et unique dans l'autre, nous avons décidé de ne pas fusionner les deux tables entre elles par souci de clarté dans notre base de données. Il nous semble plus logique et moins confus de séparer les deux tables, en particulier étant donné que les logs seront uniquement visibles par l'administrateur système. Nous avons également décidé de donner la clé étrangère de cette relation à la table Log_tickets_valides car le login de l'utilisateur demandeur est primordial pour cette table.

De plus, cette relation permets à la table Ticket de chercher son identifiant dans les logs de tickets validés et d'en récupérer la date et l'adresse IP, qui sont des informations bonnes à prendre.
